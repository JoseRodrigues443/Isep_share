...
import lapr4.red.s2.lang.n1151117.formula.lang.Assign;

/**
 * The implementation of the <code>Cell</code> interface.
 * @author Einar Pehrson
 */
public class CellImpl extends Observable implements Cell {
	 ...
	 * @param address the address of the cell
	 */
	public static Cell cellImpl(Spreadsheet spreadsheet, Address address) {
		return new CellImpl(spreadsheet, address);
	}
 ...
 */
	 ...
	 * Notifies all registered listeners that the value of the cell changed.
         * 
         * EDIT: To avoid infinite loop problems, i added a validation
         *       if the operand is the assign.
         *       Case it is, it will not notify the dependents,
         *       otherwise it will notify
	 */
	private void fireValueChanged() {
		...
		// Notifies dependents of the changed value
                if(formula instanceof Expression){
                    Expression e = formula.getExpression();
                    if(e instanceof NaryOperation){
                        if(!(((NaryOperation) e).getOperator() instanceof Assign)){
 ...
 */
	...
	public void setContent(String content) throws FormulaCompilationException {
        ...
        public void setContentSorted(String content) throws FormulaCompilationException{
            if(!this.content.equals(content)){
                	storeContent(content);
			fireContentChanged();
			reevaluate();
		...
		if (formula != null)
			...
			for (Reference reference : formula.getReferences())
				for (Cell precedent : reference.getCells()) {
					if (!this.equals(precedent) && !precedents.contains(precedent)) {
 ...
 */
	...
	private void writeObject(ObjectOutputStream stream) throws IOException {
        ...
        public void notifyObs(){
            setChanged();
            notifyObservers(this.getAddress());
        }
}
