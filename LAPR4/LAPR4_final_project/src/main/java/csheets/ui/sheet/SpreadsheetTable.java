    ...
    public SpreadsheetTable(SpreadsheetTableModel tableModel, UIController uiController) {
        ...
        getTableHeader().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent mouseEvent) {
                ...
                int columnIndex = convertColumnIndexToModel(columnAtPoint(mouseEvent.getPoint()));
                if (columnIndex >= 0) {
                    uiController.setActivateColumn(columnIndex);
                    if (uiController.getSortingOrder()) {
                        JOptionPane.showMessageDialog(null, "Ascending Sorting from cell " + 
                                uiController.getActiveSpreadsheet().getCell(columnIndex, 0).getAddress().toString().substring(0, 1), "Sorting Information", JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(null, "Descending Sorting from cell " + 
                                uiController.getActiveSpreadsheet().getCell(columnIndex, 0).getAddress().toString().substring(0, 1), "Sorting Information", JOptionPane.INFORMATION_MESSAGE);
                    }
                    Cell[] tmp = uiController.getActiveSpreadsheet().getColumn(columnIndex);
                    for (Cell cell : tmp) {
                        CellImpl cellImp = (CellImpl) cell;
                        cellImp.notifyObs();
//                        int temp = 0;
//                        while(temp < 500000000) //Control variable to stop the threads to interfere with each other
//                        {
//                            temp++; 
//                        }
                    }
 ...
 * DATA
    ...
    public void setModel(TableModel dataModel) {
        ...
        StylableSpreadsheet styleableSpreadsheet = (StylableSpreadsheet) spreadsheet.getExtension(StyleExtension.NAME);
