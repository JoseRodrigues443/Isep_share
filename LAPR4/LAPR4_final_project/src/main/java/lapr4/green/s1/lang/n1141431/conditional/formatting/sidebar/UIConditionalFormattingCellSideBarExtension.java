/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package lapr4.green.s1.lang.n1141431.conditional.formatting.sidebar;

import csheets.ext.Extension;
import csheets.ui.ctrl.UIController;
import csheets.ui.ext.CellDecorator;
import csheets.ui.ext.TableDecorator;
import csheets.ui.ext.UIExtension;
import java.awt.GridLayout;
import javax.swing.BorderFactory;
import javax.swing.Icon;
import javax.swing.JComponent;
import javax.swing.JMenu;
import javax.swing.JPanel;
import javax.swing.JTextArea;
import javax.swing.JToolBar;
import javax.swing.border.TitledBorder;

/**
 * this class is used to create a new sidebar
 *
 * @author Pedro Oliveira 1141431
 */
public class UIConditionalFormattingCellSideBarExtension extends UIExtension {

    private ConditionalFormattingCellSideBarController controller;
    /**
     * A side bar that provides editing of comments
     */
    private JComponent sideBar;
    private JTextArea txt;

    public UIConditionalFormattingCellSideBarExtension(Extension extension, UIController uiController) {
        super(extension, uiController);
        // TODO Auto-generated constructor stub
    }

    /**
     * Returns an icon to display with the extension's name.
     *
     * @return an icon with style
     */
    public Icon getIcon() {
        return null;
    }

    /**
     * Returns an instance of a class that implements JMenu. In this simple case
     * this class only supplies one menu option.
     *
     *
     * @return a JMenu component
     */
    public JMenu getMenu() {
        return null;
    }

    /**
     * Returns a cell decorator that visualizes the data added by the extension.
     *
     * @return a cell decorator, or null if the extension does not provide one
     */
    public CellDecorator getCellDecorator() {
        return null;
    }

    /**
     * Returns a table decorator that visualizes the data added by the
     * extension.
     *
     * @return a table decorator, or null if the extension does not provide one
     */
    public TableDecorator getTableDecorator() {
        return null;
    }

    /**
     * Returns a toolbar that gives access to extension-specific functionality.
     *
     * @return a JToolBar component, or null if the extension does not provide
     * one
     */
    public JToolBar getToolBar() {
        return null;
    }

    /**
     * Returns a side bar
     *
     *
     */
    public JComponent getSideBar() {
        if (sideBar == null) {

            sideBar = new JPanel(new GridLayout(2, 1));

            sideBar.setName(ConditionalFormattingCellSideBarExtension.NAME_OF_EXTENSION);
            // Creates components
            txt = new JTextArea();

            // Adds borders
            TitledBorder border = BorderFactory.createTitledBorder("Conditional Cell Formatted");
            border.setTitleJustification(TitledBorder.CENTER);

            // Creates side bar
            sideBar.add(txt);
        }
        return sideBar;
    }

    /**
     * method used to put the last condition made in the JTextArea of the
     * sideBar
     */
    public void setConditionText() {
        controller = new ConditionalFormattingCellSideBarController(uiController);
        controller.setText(txt);

    }

}
