...
import lapr4.blue.s3.lang.n1150433.formula.compiler.Formula4BaseVisitor;
...
//                if (ctx.getChild(1).getChildCount() == 0) { // Conclude that operator is on the right
    ...
    @Override
    public Expression visitMoney_expression(Formula4Parser.Money_expressionContext ctx) {
    ...
    @Override
    public Expression visitMoneys(Formula4Parser.MoneysContext ctx) {
        try {
    ...
    @Override
    public Expression visitMoney_operation(Formula4Parser.Money_operationContext ctx) {
    ...
    public Expression visitEval(Formula4Parser.EvalContext ctx){
        if(ctx.EVAL() != null){
            try{
                Expression exp = new Literal(new Value());
                Expression expressions[] = new Expression[(ctx.getChildCount() / 2) - 1];
                //Starts at 3 to ignore the eval "tag", the left bracket and the quote
                for (int nChild = 3, i = 0; nChild < ctx.getChildCount(); nChild+= 2, i++) {
                    expressions[i] = visit(ctx.getChild(nChild));
                }
                double result  = expressions[0].evaluate().toDouble();
                //Does all the operations in sequence starting with the first parameter
                for (int i = 1; i < expressions.length; i+=2) {
                    switch (expressions[i].evaluate().toString()){
                        case "+" :
                            //goes to the next position so that it goes to the number instead of the operand
                           result = result + expressions[i+1].evaluate().toDouble();
                        case "-" :
                            result = result - expressions[i+1].evaluate().toDouble();
                        case "*":
                            result = result * expressions[i+1].evaluate().toDouble();
                    ...
                    exp = new Literal(new Value(result));
                }
                return exp;
    ...
    @Override
    public Expression visitWhile_loop(Formula4Parser.While_loopContext ctx){
    ...
    public Expression visitLiteral(Formula4Parser.LiteralContext ctx) {
        ...
        } else if (t.getType() == Formula4Parser.VAR) {
